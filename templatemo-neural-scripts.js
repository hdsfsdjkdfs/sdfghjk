...
// تحديد الصفحة النشطة مباشرة عند تحميل الصفحة
document.addEventListener('DOMContentLoaded', function() {
    // الحصول على اسم الصفحة الحالية
    let currentPage = window.location.pathname.split('/').pop();
    if (!currentPage) currentPage = 'index.html';
    
    // العثور على جميع الروابط في الصفحة
    const allLinks = document.querySelectorAll('a[href]');
    
    // إزالة الكلاس النشط من جميع الروابط أولاً
    allLinks.forEach(link => link.classList.remove('active'));
    
    // إضافة الكلاس النشط للروابط المطابقة
    allLinks.forEach(link => {
        if (link.getAttribute('href') === currentPage) {
            link.classList.add('active');
            console.log('تم تنشيط الرابط:', link.href);
        }
    });
});
...
function updateActiveLinks() {
    ...
    // حذف active class من جميع الروابط
    document.querySelectorAll('a').forEach(link => {
        link.classList.remove('active');
    });

    // إضافة active class للروابط المطابقة
    document.querySelectorAll('a').forEach(link => {
...
// إغلاق القائمة المتحركة عند النقر على الروابط
document.querySelectorAll('.mobile-nav a').forEach(link => {
    link.addEventListener('click', () => {
        mobileMenuToggle.classList.remove('active');
        mobileNav.classList.remove('active');
        // تحديث الروابط النشطة بعد النقر
...
document.addEventListener('click', (e) => {
    if (!mobileMenuToggle.contains(e.target) && !mobileNav.contains(e.target)) {
        ...
        function setActivePage() {
            ...
            // حذف كل الفئات النشطة أولاً
            document.querySelectorAll('.nav-links a, .mobile-nav a, .footer-links a').forEach(link => {
                link.classList.remove('active');
            });
            
            // إضافة الفئة النشطة للروابط المطابقة
            document.querySelectorAll('.nav-links a, .mobile-nav a, .footer-links a').forEach(link => {
                const href = link.getAttribute('href');
                // إذا كنا في صفحة matrix.html وكان الرابط يشير إلى matrix.html
                if (currentPage === href || 
                    (currentPage === 'matrix.html' && href === 'matrix.html') ||
                    (currentPage === 'features.html' && href === 'features.html') ||
            ...
            console.log('Current page:', currentPage); // للتأكد من الصفحة الحالية
        }

        // تشغيل الدالة عند تحميل الصفحة
        window.addEventListener('load', setActivePage);
        // تشغيل الدالة عند تغيير المسار
